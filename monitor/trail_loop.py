...
from config import (
    ...
    TRAIL_SL_BREACH_HOLD_SECS,
    BAR_CLOSE_SL_EVAL,
    TP_HARD_EXIT,
    MAX_EXIT_SLIPPAGE_ATR_PCT,
    TRAIL_STAGE_UPGRADE_MODE,
)
...
class TrailMonitor:
    ...
    async def _evaluate_tick(self, price: float,  source: str = "other") -> None:
        ...
        # ── 3b. Intrabar stage upgrade — GOLDBOT toggle ───────────────────────
        # Pine-exact behavior upgrades trailStage AT BAR CLOSE only
        # (calc_on_every_tick=false) — see on_bar_close() step 3. Set
        # TRAIL_STAGE_UPGRADE_MODE=bar_close in .env to keep that Pine-exact
        # timing. Default here is "intrabar" per explicit user request to
        # react on the running candle; this is a no-op unless that mode is
        # active (see _intrabar_stage_upgrade_check).
        self._intrabar_stage_upgrade_check(state, risk, price, is_long, atr)
    ...
    async def _evaluate_tick_sl_only(self, price: float) -> None:
        ...
        # ── 1b. Intrabar stage upgrade — GOLDBOT toggle ──────────────────────
        # Use state.best_price (the authoritative running peak) rather than
        # this tick's raw price, since this evaluator intentionally skips
        # updating best_price itself (see docstring above) and may receive
        # stale Binance-offset prices.
        self._intrabar_stage_upgrade_check(state, risk, state.best_price, is_long, atr)
    ...
    def _intrabar_stage_upgrade_check(
        self,
        state   : TrailState,
        risk    : RiskLevels,
        price   : float,
        ...
        """
        GOLDBOT — user-requested: stage upgrade + BE evaluated on the RUNNING
        (unclosed) candle, not just at bar close.

        Only active when config.TRAIL_STAGE_UPGRADE_MODE == "intrabar"
        (the default for this build). Mirrors on_bar_close() steps 3 & 4
        exactly, using the live tick price as the profit-distance anchor
        instead of waiting for bar_close.

        This is a deliberate trade-off, not a silent revert of the
        FIX-C/FIX-D Pine-parity fix: set TRAIL_STAGE_UPGRADE_MODE=bar_close
        in .env to restore the original Pine-exact (bar-close-only) timing.
        """
        if TRAIL_STAGE_UPGRADE_MODE != "intrabar":
            return

        profit_dist = (price - risk.entry_price) if is_long else (risk.entry_price - price)
        ...
        if new_stage > state.stage:
            ...
            if getattr(state, 'trail_armed', False):
                new_trail_sl = _trail_sl_from_best(state.best_price, state.stage, atr, is_long)
                self._apply_trail_sl(state, risk, new_trail_sl, is_long, source="stage_upgrade_intrabar")

        # Breakeven
        if not state.be_done and profit_dist > atr * BE_MULT:
            self._activate_be(state, risk, is_long, atr, source="intrabar")
