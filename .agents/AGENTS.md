...
3. **VPS Commands Parity:** When guiding the user on VPS deployment, always provide the standard sequence:
   ...
   ```
4. **Dashboard Parity:** Maintain neon green monospace `#00ff66` typography, IST time formatting, 100% mobile screen fit (zero horizontal body scroll), extended Equity Curve chart height (`340px`), `Qty` column in the trade history table, and live unrealized P&L row in the POSITION card below the stage bar.
