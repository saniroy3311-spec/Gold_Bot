...
from config import PAPER_TRADING, PAPER_TRADING_BALANCE
...
# ── Live engine state (GOLDBOT single-screen dashboard) ───────────────────────
# Ephemeral signal-engine snapshot — indicators, filters, sizing — that only
# exists in memory during bar processing. main.py calls update_live_state()
# after each bar close and after each entry-sizing calc. Never persisted;
# always reflects the most recent evaluation, regardless of open position.
_live_state: dict = {
...
def update_live_state(**kwargs) -> None:
    """Merge fields into the live-state snapshot. Call from main.py."""
...
class _Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/status":
            ...
            try:
                today = _journal.get_daily_summary() if _journal else {}
            ...
            base_cap = PAPER_TRADING_BALANCE if PAPER_TRADING else (_live_state.get("equity_usd") or 2000.0)
            self._send_json({
                **_live_state,
